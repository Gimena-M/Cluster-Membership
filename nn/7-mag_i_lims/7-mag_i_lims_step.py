...
lims = [0, 18, 19, 20, 21, 22, 23, 24, 25, 99]
...
for i in range(len(lims) - 1):
    ...
    for feat in features:
        ...
        mod.main(model_exists= True, test_name= f'{feat}/{lims[i]}_to_{lims[i+1]}')
        ...
        mod.main(model_exists= True, resume_training= True, test_name= f'{feat}/{lims[i]}_to_{lims[i+1]}--retrained', retrain_name= f'{feat}/{lims[i]}_to_{lims[i+1]}--retrained')
...
with open('metrics_2.csv', 'w', newline= '') as file:
    writer = csv.writer(file)
    writer.writerow(['Features', 'i_min', 'i_max', 'Precision', 'Recall', 'Specificity', 'Accuracy', 'F1-score', 'PR AUC', 'ROC AUC', 'Threshold', 'Precision (Ret)', 'Recall (Ret)', 'Specificity (Ret)', 'Accuracy (Ret)', 'F1-score (Ret)', 'PR AUC (Ret)', 'ROC AUC (Ret)', 'Threshold (Ret)'])
