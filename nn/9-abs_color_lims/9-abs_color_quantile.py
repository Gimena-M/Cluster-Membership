...
q1 = np.quantile(data.data['ri_abs'], 0.25)
q3 = np.quantile(data.data['ri_abs'], 0.75)
...
for i,nam in enumerate(names):
    ...
    d = data.copy()
    if lims[i+1]:
        d.feat_max = {"ri_abs": lims[i+1]}
    if lims[i]:
        d.feat_min = {"ri_abs": lims[i]}
    d.prep()
    ...
    for feat in features:
        ...
        if nam != 'all':
            mod.main(model_exists= True, resume_training= True, test_name= f'quantile/{feat}/{nam}--retrained', retrain_name= f'quantile/{feat}/{nam}--retrained', importances = None)
