...
lims = [0, 9, 9.5, 10, 10.5, 11, 11.5, 12, 99]
...
for i in  range(len(lims) - 1):
    ...
    for feat in features:
        ...
        mod.main(model_exists= True, test_name= f'step/{feat}/{lims[i]}_to_{lims[i+1]}')
        ...
        mod.main(model_exists= True, resume_training= True, test_name= f'step/{feat}/{lims[i]}_to_{lims[i+1]}--retrained', retrain_name= f'step/{feat}/{lims[i]}_to_{lims[i+1]}--retrained')
...
with open('metrics_step.csv', 'w', newline= '') as file:
    ...
    writer.writerow(['Features', 'mass_min', 'mass_max', 'Precision', 'Recall', 'Specificity', 'Accuracy', 'F1-score', 'PR AUC', 'ROC AUC', 'Threshold', 'Precision (Ret)', 'Recall (Ret)', 'Specificity (Ret)', 'Accuracy (Ret)', 'F1-score (Ret)', 'PR AUC (Ret)', 'ROC AUC (Ret)', 'Threshold (Ret)'])
