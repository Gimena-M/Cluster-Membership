...
# data = DataHandler(validation_sample= True, features_txt= 'all_features.txt', fields_list=['W02'], balance= 'weights')
# data.main()
...
#         print ('-'*70)
#         print(f'MODEL: {name}')
#         mod = NNModelController(layers= layers, name= 'all_run1/' + name, data= data, compile_params= compile_params, epochs= 1000, batch_size= 2000)
#         mod.main(read_data= False, prep_data= False)
...
# df.to_csv('search_summary_1.csv', index= False, mode= 'a')


# 2nd run
# complex networks with dropout did better
# archs = dict(
#     n_layers = [2, 3, 4], 
...
#     # possible architectures for this n_layers
#     combinations_lay = list(product(*[archs['n_units']]*n_lay))
#     # drop combinations without at least n_lay * 256 neurons
#     combinations_lay = [comb for comb in combinations_lay if np.sum(comb)>=256*n_lay]
...
#         print ('-'*70)
#         print(f'MODEL: {name}')
#         mod = NNModelController(layers= layers, name= 'all_run2/' + name, data= data, compile_params= compile_params, epochs= 1000, batch_size= 2000)
#         mod.main(read_data= False, prep_data= False)
...
#     df = pd.DataFrame(dict(model = names, loss = losses, pr_auc = pr_auc, roc_auc = roc_auc, f1 = f1s))
#     df.to_csv('search_summary_2.csv', index= False, mode= 'a')
